...
Shader "Unity Shaders Book/Chapter 7/NormaMapWorldSpace"{
    Properties {
    ...
    SubShader{
        Pass{
            ...
            struct v2f{
                ...
                float4 uv: TEXCOORD;
                // 一个插值寄存器最多只能存储float4 大小的变批，对于矩阵，可以把它们按行拆成多个向量再进行存储。
                // 代码中的TtoWO 、TtoWl 和TtoW2 就依次存储了从切线空间到世界空间的变换矩阵的每一行。
                // 为了充分利用插值寄存器的存储空间，我们把世界空间下的顶点位置存储在这些变量的w中。
